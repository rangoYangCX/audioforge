...
def test_settings_dialog_shows_autosave_controls(monkeypatch) -> None:
    monkeypatch.setattr(RecoveryService, "has_snapshot", lambda self: False)

    controller = MainController()
    controller.window.open_settings_dialog()
    controller.window.set_autosave_preferences(enabled=True, interval_minutes=7)
    controller.window.set_autosave_history([
        {"label": "2025-01-01T00:00:00Z | demo.afproj", "path": "/tmp/demo.json", "detail": "demo"},
    ])
    QApplication.processEvents()

    assert controller.window.settings_dialog.isVisible()
    assert controller.window.autosave_enabled_check.isChecked() is True
    assert controller.window.autosave_interval_spin.value() == 7
    assert controller.window.autosave_history_list.count() == 1

    controller.window.settings_dialog.close()
    controller.window.close()


def test_save_project_uses_directory_selection_as_target_folder(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(RecoveryService, "has_snapshot", lambda self: False)

    controller = MainController()
    controller.project.file_path = ""
    monkeypatch.setattr(controller.window, "ask_save_project_path", lambda suggested_name: str(tmp_path))

    saved = controller.save_project()
    expected_path = tmp_path / f"{controller.project.name}.afproj"

    assert saved is True
    assert expected_path.exists()
    assert Path(controller.project.file_path) == expected_path

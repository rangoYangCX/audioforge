...
- 当前验证基线：仓库已固定为分层测试入口；日常回归默认走 `python tools/run_test_suite.py fast` / `smoke` / `main-controller`，发布级检查走 `python tools/run_full_chain_check.py`，如需全量 Python 回归再显式加 `--pytest-all`。
...
```bash
python tools/run_test_suite.py fast
...
推荐基线顺序：先定义本轮边界，再跑最窄的 `tools/run_test_suite.py` 分层入口；跨模块主链路改动再跑 harness smoke；发布级改动再跑 `run_full_chain_check.py`。`run_full_chain_check.py` 默认只带 smoke pytest 目标，只有发布前需要完整 Python 扫描时才加 `--pytest-all`。每次收口都要同步检查文档变化，固定步骤见 `docs/internal/architecture/audioforge_开发完成定义与文档同步基线.md`。

最近一次仓库内执行结果：

- `tools/run_test_suite.py smoke`：测试入口固定为仓库 smoke 集合
- `tools/run_test_suite.py main-controller`：`MainController` 实验 / 预览 / 构建回归固定为稳定批次
- `tools/run_internal_release_validation.py`：PASS
- `tools/run_full_chain_check.py`：默认包含 smoke pytest + harness + 导出契约检查
...
3. `CHANGELOG.md`
4. `开发文档.md`
5. `docs/internal/architecture/audioforge_开发完成定义与文档同步基线.md`
6. `docs/unity/UnitySDK对接规范.md`
7. `docs/unity/architecture/UnityRuntime三期GameSync设计.md`
8. `docs/internal/architecture/audioforge_第三期RTPC-State-Switch路线图.md`
9. `unity_package/README.md`
10. `unity_validation/README.md`
11. `docs/operations/internal_release_execution_plan.md`

...
- 运行时现状：仓库附带独立维护的 Unity 集成包与空项目验证材料，当前已支持 SchemaVersion 3、AudioObjects、Events[AudioId]、GameParameters/StateGroups/SwitchGroups、Audio/Bus 级 GameSync 绑定、emitter context、全局 State、全局/局部 Game Parameter 与 child effects smoke。
- 当前验证基线：2026-05-12 的完整发布基线仍为 `pytest` 112 项通过、真实 WAV 烟雾工程 PASS、全链路检查 4/4 通过；本轮 phase3 聚焦回归额外覆盖了 serializer/exporter/preview/layout 共 25 项通过，并完成 Unity package / validation 关键 C# 文件诊断无错误。
...
- 新增 `docs/UnitySDK输出规范.md`，正式冻结 Unity SDK 的 UPM 输出骨架：后续交付统一按 `com.audioforge.runtime` 根目录结构生成 `package.json`、`README.md`、`Runtime/`、`Editor/` 和 `Documentation~/`。
- Unity SDK 打包链已把规范文档一并带入包内 canonical 文档；Windows 桌面发布目录现在也会额外内嵌 `SDK/com.audioforge.runtime/`，便于直接交给 Unity 程序做本地包接入。
- 片段编辑台布局已专项收口：波形编辑区不再被下方操作按钮压缩，下方操作区改为更短更清晰的分层布局，批量编辑反馈卡片压缩为轻量提示条。
- 源音频树默认选择语义已从批量优先改为类资源管理器的 `ExtendedSelection`，单击默认单选，仍保留 `Ctrl` / `Shift` 多选和现有批量操作能力。
- 这轮发布建议重新生成 `dist/AudioForgeUnityPackage-0.09.1/`，把最新 SDK 输出规范、Schema 3 契约说明、验证报告和 release note 一起交付给 Unity 同学。
...
- Unity runtime 已支持 emitter handle、SetState、SetSwitch、SetGameParameter、Schema v3 初始化、Bus GameSync 求值和 validation smoke；详细设计与当前边界见 `docs/UnityRuntime三期GameSync设计.md`。
...
- `docs/UnitySDK输出规范.md`：Unity SDK 输出骨架与打包规范
...
2. `docs/UnitySDK输出规范.md`
3. `CHANGELOG.md`
4. `开发文档.md`
5. `docs/UnityRuntime三期GameSync设计.md`
6. `docs/internal/audioforge_第三期RTPC-State-Switch实施计划.md`
7. `docs/Unity场景联调清单.md`
8. `unity_package/README.md`
9. `unity_validation/README.md`
10. `docs/WSG_audiotest.md`
11. `docs/internal/internal_release_execution_plan.md`

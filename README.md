...
AudioForge 是一个面向 Unity 休闲游戏项目的数据驱动音频工具。当前仓库在保留 phase2 编辑器交付基线的同时，已经把 phase3 的 Wwise 风格 RTPC、State、Switch 主链落到当前代码线上：项目级 Game Sync 模型、Schema v2、试听求值、Unity runtime 控制层和验证链路都已打通。
...
- 阶段状态：phase2 的对象浏览器三分页、绑定弹窗、OneShot 与 UPM SDK 交付仍然保留；phase3 已新增 GameSync 工作区、事件与总线级 RTPC/State/Switch 绑定、Schema v2 导出和 Unity runtime GameSync 消费能力。
...
- 运行时现状：仓库附带独立维护的 Unity 集成包与空项目验证材料，当前已支持 SchemaVersion 2、GameParameters/StateGroups/SwitchGroups、事件与总线的 RtpcBindings/StateOverrides、事件 SwitchVariants、emitter context、全局 State、全局/局部 Game Parameter 与 child effects smoke。
...
- Unity runtime 已支持 emitter handle、SetState、SetSwitch、SetGameParameter、Schema v2 初始化、Bus GameSync 求值和 validation smoke；详细设计与当前边界见 `docs/UnityRuntime三期GameSync设计.md`。

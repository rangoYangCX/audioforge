...
- 总线工作区新增路由图，按“当前总线 -> 上游总线 -> Master / 下游分支 / 作者态输出”显示当前路由链，并支持从图上直接跳转到相邻总线。
- 资源工作区新增批量编辑反馈卡，会保留当前事件最近一次批量权重、批量属性、批量重命名、排序或拖拽重排的摘要、影响数量和字段变化。
- 这次维护更新只涉及桌面工具的编辑反馈与导航，不修改 Unity 导出契约、资源目录结构或运行时读取边界。
...
- 总线工作区已新增路由图，可同时回看当前总线、上游链路、下游分支和作者态输出百分比。

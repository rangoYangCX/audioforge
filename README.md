...
- 工具端现状：已具备 AppShell 主壳层、顶部应用栏、左侧任务导航、欢迎页、六大主工作区、结果中心、问题中心、响度监视器、总线编辑、自动恢复和高频快捷操作，以及事件树多选、批量改总线、批量重命名、批量删除与搜索定位。
- 运行时现状：仓库附带独立维护的 Unity 集成包与空项目验证材料，支持 Event Id 搜索、从 `AudioData.json` 刷新 `AudioEventID.cs`，以及 Unity AudioMixer 对接。
- 当前验证基线：`pytest` 62 项通过；真实 WAV 烟雾工程 PASS；全链路检查 4/4 通过。
...
- 新增 Unity 场景级联调清单，便于把空工程验证推进到目标项目签收。
- 桌面端与 Unity 侧交付链路收口到新的 0.03 桌面发版。
- Unity 运行时交付内容已包含 Event Id 搜索增强、`AudioData.json` 到 `AudioEventID.cs` 刷新菜单，以及 AudioMixer 音量联调能力。
...
2. `docs/Unity场景联调清单.md`
3. `unity_package/README.md`
4. `unity_validation/README.md`
5. `开发文档.md`
6. `docs/WSG_audiotest.md`
7. `docs/internal/internal_release_execution_plan.md`

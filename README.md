...
- 当前验证基线：最近一次完整基线为 `pytest` 67 项通过、真实 WAV 烟雾工程 PASS、全链路检查 4/4 通过；0.06.0 交付前额外通过主窗口布局回归 41/41。
...
- 结果中心新增诊断概览页，`MainController` 会统一收口最近日志、校验、构建、响度和 Bus 状态，并以结构化诊断 section 回流到主界面。
- 构建交付页开始直接消费控制器的 build metadata，新增构建画像列表与详情；结果坞、结果中心和活动摘要不再各自维护重复状态。
- 顶部命令面板、全局搜索和工作区入口继续收敛，显式按钮不再被重复暴露；Bus 路由图与资源页批量反馈也一并并入当前工作流。
- 修复工作区在事件设计、构建交付、校验修复和结果中心之间切换时主编辑区被挤窄的问题，避免内页签布局错乱。
- Unity 导出契约、资源目录结构和运行时 SDK 接口保持不变；若需要新的文档副本、验证材料或示例导出物，需重新生成 `dist/AudioForgeUnityPackage-0.06.0/`。

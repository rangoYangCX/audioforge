...
- 当前验证基线：最近一次完整基线为 `pytest` 76 项通过、真实 WAV 烟雾工程 PASS、全链路检查 4/4 通过；主窗口工作台聚焦回归已覆盖欢迎页直达入口、结果坞紧凑/展开和恢复默认布局行为。
...
- 欢迎页继续收敛为任务型首页，保留新建工程、打开工程以及直达事件设计、资源整理、Bus 混音台和结果中心的入口，不再把说明性内容堆成双列首页。
- 底部结果坞改为默认紧凑、按需展开；需要更多细节时再展开查看，完整回看仍统一进入结果中心，减少常驻占位。
- 工作区 overview、note card 与状态条继续去重，重复说明和重复状态被收回到已有工作区上下文、结果中心和结果坞摘要里。
- 文档、签收摘要和全链路报告已按最新 smoke 结果刷新，当前仓库验证基线更新为 76 项测试通过。
- Unity 导出契约、资源目录结构和运行时 SDK 接口保持不变；若需要新的文档副本、验证材料或示例导出物，需重新生成 `dist/AudioForgeUnityPackage-0.06.1/`。

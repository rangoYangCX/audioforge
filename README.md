...
- 项目级 `AudioObject` 已成为正式主模型：Event 只保留 `MaxInstances`、`CooldownSeconds`、`StealPolicy`、`Notes` 和 `AudioId`，Bus、PlayMode、Clips、GameSync 绑定和声音属性统一收口到 Audio Object。
- `AudioData.json` 已升级为 `SchemaVersion = 3`：顶层正式输出 `AudioObjects` 与 `Events[AudioId]`，不再导出嵌套 `Audio` 或扁平镜像字段。
- Unity runtime、JSON 适配器和全链路检查已切到 Schema 3 单一路径：缺失 `AudioId`、缺失目标 `AudioObject` 或字段不完整都会被直接判为契约错误。
- 对象浏览和导入流程已跟随主模型重排：Audio 树成为一等编辑入口，源音频树按“引用 Audio 数”统计，外部拖拽导入按事件树 / Audio 树 / 源音频树三条规则收口。
- Bus 混音台工作区已重构为“当前 Bus / 工程总览 / Bus GameSync”三分页，当前 Bus 再细拆成“路由 / 电平与导出”二级页签，减少单页堆叠与误操作。
- 这轮发布建议重新生成 `dist/AudioForgeUnityPackage-0.09.0/`，把新的 Schema 3 契约说明、验证报告和 release note 一起交付给 Unity 同学。

...
- 当前验证基线：最近一次完整基线为 `pytest` 87 项通过、真实 WAV 烟雾工程 PASS、全链路检查 4/4 通过；主窗口工作台聚焦回归已覆盖欢迎页直达入口、结果坞紧凑/展开、最近试听修复，以及窗口偏好持久化回环。
...
- 最近试听侧栏卡片收口成更明确的 transport card，标题、状态徽标、控制按钮和响度摘要现在按播放器卡片的层级排布。
- 最近试听按钮进一步放大成更易点击的圆形图标按钮，播放中、暂停、可试听和可重播状态都能直接在卡片顶部识别。
- 最近试听卡片与响度监视器的摘要标签已经解耦，侧栏卡片不会再因为视图重挂载而丢失内部指标。
- Unity 导出契约、资源目录结构和运行时 SDK 接口保持不变；若需要新的文档副本、验证材料或示例导出物，需重新生成 `dist/AudioForgeUnityPackage-0.06.2/`。

...
- 统一使用说明：`docs/AudioForge使用说明.md`
- Unity 端对接主文档：`docs/UnitySDK对接规范.md`
- 工具总体设计与边界：`开发文档.md`
...
- 工程保存已升级为可迁移模式：保存 `.afproj` 时会把工程引用到的源音频一起收纳到同名工程目录 `ProjectName/Sources/`，工程文件内统一写相对路径；把 `.afproj` 和同名目录一起迁移后，重新打开仍可继续试听、校验和构建。
- 相对导出目录现已锚定到工程文件位置，而不是当前进程工作目录；同一个工程在不同机器或不同目录下重新打开后，构建仍会落到工程预期的导出目录。
- mac 打包链已补显式运行库收集：`tools/build_macos_app.py` 会随 `.app` 一起收集 `pygame`、`numpy`、`scipy`、`soundfile`，降低打包后本地试听因依赖缺失直接失效的概率。
- mac 启动已补 Qt 高 DPI 与样式收口，真实试听链路也增加了 `pygame.mixer` 多档初始化兜底与失败提示；已知风险从“静默没声音 / UI 比例异常”收敛为“需要在 mac 真机继续确认最终显示与 CoreAudio 行为”。

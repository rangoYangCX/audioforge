...
- 当前仓库级验证基线：`pytest` 62 项通过，真实 WAV 烟雾工程 PASS，全链路检查 4/4 通过。
- 当前结果：PASS。

> 注：本样板工程本身的导出与检查快照保留生成当日结果，用于说明样板工程覆盖面；仓库最新基线请以 `reports/internal_release_smoke/checks/full_chain_report.md` 为准。

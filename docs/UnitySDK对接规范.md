...
> 当前文档同步日期：2026-05-11。
> 当前工具版本：AudioForge 0.06.0。

0.1 版本增量

- AudioForge 0.06.0 新增：结果中心接入“诊断概览”页，桌面工具会把最近日志、校验、构建、响度和 Bus 上下文统一收口到结构化诊断快照；Unity 运行时契约不变。
- AudioForge 0.06.0 新增：构建交付页开始直接显示构建画像，最近一次构建范围、资源差异和交付目标会在工具端页面中直接展开；Unity 侧无需消费新增字段。
- AudioForge 0.06.0 修复：桌面工具在切换事件设计 / 构建交付 / 校验修复 / 结果中心时的主工作区宽度回归，避免页签布局错乱；Unity SDK 接口不受影响。
...
- 最近一次仓库内完整验证结果：`pytest` 67 项通过；真实 WAV 烟雾工程 PASS；全链路检查通过；0.06.0 交付前额外通过主窗口布局回归 41/41。
...
2026-05-11 版本说明：当前这轮 0.06.0 发布不会自动改动已经生成好的 Unity SDK 包。只有在你需要同步新版文档副本、验证材料或重新导出的样例时，才需要重新执行打包流程生成 `dist/AudioForgeUnityPackage-0.06.0/`；若不重新打包，现有 SDK 包结构、接口和运行时代码保持不变。
...
如果 Unity 同学已经拿到了之前生成的包，可以直接继续对接；0.06.0 主要改善的是桌面工具诊断收口、编辑工作台体验和布局稳定性，而不是 Unity SDK 接口。

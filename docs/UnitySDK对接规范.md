...
- 最近一次仓库内完整验证结果：`pytest` 76 项通过；真实 WAV 烟雾工程 PASS；全链路检查 4/4 通过；smoke 报告与签收摘要已在 2026-05-11 刷新。
...
2026-05-11 版本说明：当前这轮 0.06.1 发布不会自动改动已经生成好的 Unity SDK 包。只有在你需要同步新版文档副本、验证材料或重新导出的样例时，才需要重新执行打包流程生成 `dist/AudioForgeUnityPackage-0.06.1/`；若不重新打包，现有 SDK 包结构、接口和运行时代码保持不变。
...
如果 Unity 同学已经拿到了之前生成的包，可以直接继续对接；0.06.1 主要改善的是桌面工具工作台信息收口、结果坞表现和交付文档，而不是 Unity SDK 接口。
